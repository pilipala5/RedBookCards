...
 
问题描述
目前我发现，导出图片的时候，预览组件有时候虽然我点击的是适应窗口模式，但是导出图片后，会切换成实际大小模式
期望输出
...
# src/ui/preview_widget.py - Qt兼容优化版
# ============================================
...
### 系统:
看了您的代码，问题出在 `preview_widget.py` 的 `export_pages` 方法中。导出时会调用 `self.web_view.setFixedSize(1080, 1440)`，这会改变 WebView 的尺寸设置，但导出完成后没有恢复原来的模式设置。

## 问题分析

在 `export_pages` 方法中，`ImageExporter` 会设置 WebView 为固定尺寸：
```python
# 确保WebView是固定尺寸
self.web_view.setFixedSize(1080, 1440)
```

但导出完成后，没有根据当前的预览模式恢复相应的设置。

## 解决方案

需要在 `preview_widget.py` 中修改两个地方：

### 1. 在 `export_pages` 方法开始时保存当前模式状态

### 2. 在 `on_export_finished` 方法中恢复模式状态
...
1. **添加导出状态标志** (`__init__` 方法)
   - 添加 `self._is_exporting = False` 标志来追踪导出状态

2. **阻止导出期间的模式切换** (`on_mode_changed` 方法)
   - 导出时不允许切换预览模式

3. **阻止导出期间的页面更新** (`display_current_page` 方法)
   - 导出时不更新显示内容

4. **保存和恢复状态** (`export_pages` 方法)
   - 导出开始时保存当前预览模式和页码
   - 设置 `_is_exporting = True`
   - 保存 `_saved_preview_mode` 和 `_saved_current_page`

5. **恢复原始状态** (`on_export_finished` 方法)
   - 导出完成后恢复之前的预览模式
   - 根据保存的模式重新设置所有相关的UI属性
   - 恢复到之前的页面

6. **取消导出时也恢复** (`on_export_canceled` 方法)
   - 取消导出时同样恢复原始状态

这样修改后，导出图片时会：
1. 记住当前的预览模式（适应窗口/实际大小）
2. 记住当前查看的页码
3. 导出完成或取消后，自动恢复到原来的模式和页面
4. 导出过程中防止模式切换和页面更新

这个解决方案确保了导出操作不会影响用户的预览体验。
